...
public sealed class Scanner
{
    ...
    public Token GetNextToken()
    {
        if (position_ >= input_.Length)
            return T_EOF.SetPosition(line_, column_, column_);
        ...
        return E_UNRECOGNIZED
            .SetRawText(input_.Substring(position_, Math.Min(1, input_.Length - position_)))
            .SetPosition(line_, column_, column_ + Math.Min(1, input_.Length - position_));
            ;
    }
